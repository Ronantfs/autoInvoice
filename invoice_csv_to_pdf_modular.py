...
# Configuration
INVOICES_FOLDER_PATH = r'C:\Repositories\code_repos\autoInvoice\invoices_modular'
...
def draw_static_elements(canvas, width, height, student, tutor, total_fee, month, invoice_number):
    ...
    canvas.setFont("Helvetica-Bold", 24)
    canvas.drawString(75, height - 270, f"{student} Tuition with {tutor}")
    canvas.setFont("Helvetica-Bold", 18)
    canvas.drawString(75, height - 310, f"Total Invoice: £{total_fee}")
    # Draw a horizontal line

    #bank details with divider lines
    #lines
    x_start = 75  # Start x-coordinate
    y_position = height - 320  # Y-coordinate for the line
    ...
    canvas.drawString(75, height - 335, f"Bank Details:")
    canvas.drawString(75, height - 350, "Ronan A T Friedlander")
    canvas.drawString(75, height - 365, "ronantftutors")
    canvas.drawString(75, height - 380, f"04 - 00 - 04")
    canvas.drawString(75, height - 395, f"72315333")
    canvas.setStrokeColor(ut_lgreen)
    canvas.line(x_start, 390, x_end, 390)
    canvas.setStrokeColor(colors.black)

    # footer
    canvas.setFont("Helvetica-Bold", 18)
    canvas.setFillColor(colors.lightblue)
    canvas.drawRightString(425, 50, "www.understandingtutors.com")


def create_data_table(month_df, canvas, width, height):
    ...
    table.drawOn(canvas, 75, 300)  # Adjust Y-position as needed
...
def create_invoice_pdf(student, month_df, month, tutor, total_fee, invoice_name, invoice_number):
    ...
    # Draw data table
    create_data_table(month_df, c, width, height)
...
def process_data_for_student(student, student_invoice_dfs):
    ...
    for month in student_invoice_dfs:
        ...
        # Select and rename columns for the invoice
        invoice_columns = ['Lesson Date', 'Lesson Length (Hours)', r'Hourly rate (£/hr)', 'Lesson Fee']
        month_df = month_df[invoice_columns]
        invoice_columns_rename = {
            'Lesson Date': 'Lesson Date',
            'Lesson Length (Hours)': 'Duration (Hours)',
            'Hourly rate (£/hr)': 'Hourly rate (£/hr)',
